  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 min-h-screen flex flex-col font-inter text-zinc-100">
      ...
      <div className="flex-1 grid lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="mt-8 p-6 bg-indigo-950/30 border border-indigo-500/30 rounded-2xl animate-fade-in relative">
                 ...
                 <div className="text-zinc-300 text-sm leading-relaxed whitespace-pre-wrap">
                    {currentQuestion.explanation.split('\n').map((line, i) => {
                        if (line.includes('[Traditional Method]')) {
                            return <div key={i} className="text-emerald-400 font-bold mt-2 mb-1">{line}</div>;
                        }
                        if (line.includes('[Shubham\'s Shortcut]')) {
                            return <div key={i} className="text-amber-500 font-bold mt-4 mb-1">{line}</div>;
                        }
        ...
        {/* Sidebar for Rules & Hints */}
        <div className="lg:col-span-4 space-y-6">
          {/* Strategy Rules (Always show if available) */}
          {currentQuestion.strategyRules && currentQuestion.strategyRules.length > 0 && (
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl">
              ...
              <ul className="space-y-3">
                {currentQuestion.strategyRules.map((rule, i) => (
          ...
          {/* Hints (Always show if available) */}
          {currentQuestion.hints && currentQuestion.hints.length > 0 && (
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-amber-500 font-bold uppercase tracking-widest text-xs">
                  <HelpCircle size={16} /> Tactical Hints
                </div>
                <span className="text-[10px] text-zinc-600 font-mono">{visibleHints}/{currentQuestion.hints.length}</span>
              </div>
          ...
          {subject === Subject.THINKING && !currentQuestion.strategyRules && !currentQuestion.hints && (
          ...
          {subject !== Subject.THINKING && (!currentQuestion.strategyRules || currentQuestion.strategyRules.length === 0) && (!currentQuestion.hints || currentQuestion.hints.length === 0) && (
